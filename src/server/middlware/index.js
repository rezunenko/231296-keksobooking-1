const offerSchema = require(`../routes/validation`);
...
const defaultHandler = (fn) => async (req, res, next) => {
  try {
    ...
    logger.log(`debug`, `Success request ${req.query}`);
...
const validateRequestQueryParams = (schema) => async (req, res, next) => {
  const errors = offerSchema(req.query, schema);
  if (errors.length > 0) {
...
const validateRequestBodyParams = (schema) => async (req, res, next) => {
  const errors = offerSchema(req.body, schema);
  if (errors.length > 0) {
